# -*- coding: utf-8 -*-
"""
Calculate a time series of BMU using a Euclidean distance measure for ERA-Interim data.

Created on Mon Jun  4 16:40:42 2018
...
def read_u_ECMWF(year_array):
    # read zonal velocity from ERA-INterim data
    directoryname='E:/datasets/NCAR_NCEP2/wind_time_fixed/'  
    variable_name='uwnd'
    ...
    climatology = ncid1.uwnd.groupby('time.dayofyear').mean('time')
    # calculate the anomaly from the 20 year climatology
    anomalies = ncid1.uwnd.groupby('time.dayofyear') - climatology
     # output the raw xarray (ncid1) and anomaly patterns and the climatology 
    return anomalies,climatology,ncid1


def read_v_ECMWF(year_array):
    
    # read meridional velocity from ERA-INterim data
    directoryname='E:/datasets/NCAR_NCEP2/wind_time_fixed/'  
    variable_name='vwnd'
    filename=variable_name+'_resampledu_'+str(year_array[0])+'.nc'
    print('Reading year:'+str(year_array[0]))
    ...
    # calculate day of year climatology 
    climatology = ncid1.vwnd.groupby('time.dayofyear').mean('time')
    # calculate the anomaly from the 20 year climatology
    anomalies = ncid1.vwnd.groupby('time.dayofyear') - climatology
 # output the raw xarray (ncid1) and anomaly patterns and the climatology
    return anomalies,climatology,ncid1
...
for period in range(2):
    ...
    anomalies_u,climatology_u,xarray_u=read_u_ECMWF(year_array)
    anomalies_v,climatology_v,xarray_v=read_v_ECMWF(year_array)
